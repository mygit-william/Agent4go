...
// NewZhipuAdapter 创建智谱适配器
func NewZhipuAdapter(baseURL, model, apiKey string) (*ZhipuAdapter, error) {
	client := resty.New().
...
// Chat 发送对话请求
func (a *ZhipuAdapter) Chat(messages *[]Message, tools []ToolDefinition) (Response, error) {
	if len(*messages) == 0 {
	...
	resp, err := a.client.R().
		...
		SetBody(payload).
		Post(a.baseURL + "/api/paas/v4/chat/completions")
...
// ChatStream 流式对话（暂不实现）
func (a *ZhipuAdapter) ChatStream(messages *[]Message, tools []ToolDefinition, onChunk func(string)) (string, error) {
	return "", fmt.Errorf("stream mode not implemented for zhipu")
}
